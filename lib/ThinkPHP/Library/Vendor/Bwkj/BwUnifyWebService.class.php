...
if (!defined('BW_LIB_PATH'))
   define('BW_LIB_PATH', dirname(__FILE__));

require_once BW_LIB_PATH.DIRECTORY_SEPARATOR.'lib'.DIRECTORY_SEPARATOR.'config'.DIRECTORY_SEPARATOR.'config.class.php';
require_once BW_LIB_PATH.DIRECTORY_SEPARATOR.'lib'.DIRECTORY_SEPARATOR.'util'.DIRECTORY_SEPARATOR.'util.class.php';
require_once BW_LIB_PATH.DIRECTORY_SEPARATOR.'lib'.DIRECTORY_SEPARATOR.'util'.DIRECTORY_SEPARATOR.'http.php';

date_default_timezone_set('Asia/Shanghai');
spl_autoload_register("BwUtil::_loadOfClassPhp");

class BwUnifyWebService
{	 
    ...
    {
        try {
           $class = BwUnifyWebService::$_as[$code].'WebService';
           return BwUnifyWebService::packCusInfo(
                      BwUnifyWebService::xmlUnSerializer($class::getCusInfoByNetWork($param)),$param);
    ...
    private static function xmlUnSerializer($xml = NULL) { 
        try {
            ...
            if (!xml_parse($xml_parser,$xml,true)) {
                ...
                BwUnifyWebService::checkQueryStatus($xml);
            }
            return BwUtil::makeObjToArray(simplexml_load_string($xml));
        } catch (\Exception $e) {
            throw new \Exception($e->getMessage(), $e->getCode());
        }
    ...
    private static function packCusInfo(array $data = NULL,$param = NULL) {
        ...
        $data['origin'] = BwUnifyWebService::$_map[$rc['province']['_as']];
    ...
    private function makeNickName($data = NULL) {
        return substr($data['CustName'],0,3).'auto'.BwUtil::makeRandomValue(3);
    }
...
try {
    ...
    $ret = BwUnifyWebService::getCusInfoByNetWork(
    BwUnifyWebService::analyseProvinceCodeByTax($_param['CusTaxCode']),$_param);
    print_r($ret); 
} catch (Exception $e) {
