...
class Date {
	...
	public function  getWeekday() {
		return $this->cWeekday.' '.$this->gettime();
	}
    ...
    /**
     * [autoGettimeDate 自动判断时间格式]
     * @return [type] [description]
     */
    public function autoGettimeDate() {
        $time = $this->date; 
        $nowtime = strtotime(date('Y-m-d 00:00:00',time()));
        $newdate = new Date($nowtime);
        $mis = $newdate->getweekdayNumber();
        $sub = '-'.($mis-1).'day';
        $lastWeekTime = strtotime($sub,$nowtime);
        if ($this->date >=$lastWeekTime) { 
            $nextDay = strtotime('-1 day',$nowtime);
			$nextDay = strtotime('-16 h',$nextDay);
			if ($this->date >$nextDay) 
                return $this->gettime();  
            else 
...
//$str  = '2017-07-05 23:22:38';
//$time = strtotime($str);
//$date = new Date($time);
//echo '<br>';
//echo $date->autoGettimeDate();
