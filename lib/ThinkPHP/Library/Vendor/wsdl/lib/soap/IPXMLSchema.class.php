...
class IPXMLSchema {
	...
	public function addComplexType($type, $name = false, $parent = false) {
		...
		if(strtolower(substr($type,0,6)) == 'array(' || substr($type,-2) == '[]'){
			$typeName = substr($type, 0, 6) == 'object' ? $name . 'Item' : '';
			$this->addArray($type,$complexTypeTag, $typeName);
			...
			if(strtolower(substr($type,0,6)) == 'object'){//stdClass
				$content = substr($type, 7, (strlen($type)-8));
				$properties = split(",", $content);//split the content into properties
				$properties2 = array();
				$_i = 0;
				foreach($properties as $pr) {
					if ($_i > 0) {
						$properties2[count($properties2) - 1] = $properties2[count($properties2) - 1] . ',' . $pr;
					} else {
						$properties2[] = $pr;
					}
					$bpos = preg_match_all('/\(/', $pr, $matches);
					$apos = preg_match_all('/\)/', $pr, $matches);
					$_i = $_i + $bpos;
					$_i = $_i - $apos;
				}
				foreach((array)$properties2 as $property){
					if($pos = strpos($property, "=>")){//array with keys (order is important, so use 'sequence' tag)
						$keyType = substr($property,0,($pos));
						$valueType = substr($property,($pos+2), (strlen($property)-$pos));
						$el = $this->addTypeElement($valueType, $keyType, $tag, false, $name . camelize($keyType) . 'Type');
					}else{
	 ...
	 * @return domNode
	 */
	public function addTypeElement($type, $name, $parent, $optional = false, $complexTypeName = '') {
		...
		if($t = $this->checkSchemaType(strtolower($type)))
			...
			$typeName = empty($complexTypeName) ? $type : $complexTypeName;
			if(substr($type,-2) == '[]'){
				...
				$el->setAttribute("type", "tns:".$typeName);				
				$this->addComplexType($type, $typeName);
			}
		}
		return $el;
	}
	 ...
	 */
	public function addArray($type, $parent, $complexTypeName = '') {
		...
		if($t = $this->checkSchemaType(strtolower($type)))
			...
			if(substr($type,-2) == '[]'){
				...
				$typeName = empty($complexTypeName) ? $type : $complexTypeName;
				$el->setAttribute("wsdl:arrayType", "tns:".$typeName."[]");
				$this->addComplexType($type, $typeName);	
			}
		}
		return $el;
