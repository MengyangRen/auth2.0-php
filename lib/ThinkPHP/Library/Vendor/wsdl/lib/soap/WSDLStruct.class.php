...
class WSDLStruct {
	...
	public function generateDocument(){
		...
		foreach((array)$this->services as $serviceName => $service){
			...
			foreach((array)$service->methods as $operation){
				...
				$soapheaders = $operation->getSoapHeaders();
				if(count($parameters) > 0 || count($soapheaders) > 0 || self::CREATE_EMPTY_INPUTS){
					...
					$this->addInput($this->bindingOperationTags[$serviceName][$operationName], true, $soapheaders, $messageName);
				}
	...
	private function addMessage($name, $parts){
		...
		foreach((array)$parts as $partName => $partType){
			...
			if($t = IPXMLSchema::checkSchemaType(strtolower($partType)))
				...
				$partName = substr($partName, 0, 6) == 'object' ? camelize($name) . camelize($rawPartName) . 'Type' : $partName;				
	 ...
	 */
	private function addInput($parent, $input=true, $soapheaders = null, $messageName = null){
		$name = $input ? "wsdl:input" : "wsdl:output";
		$tag=$this->addElement($name, $parent);
		if ($messageName) {
			$soapheaders = array_merge($soapheaders, $this->headers);
			foreach ($soapheaders as $header) {
				$this->addSoapHeader($header, $messageName, $tag);
			}
		}
	...
	/*<soap:header message="tns:IMessageService_SendMail_InputMessage_Headers" part="Application_Context" use="literal" /> */
	private function addSoapHeader($header, $messageName, $parent) {
		$messageName = $messageName . "Header" . $header->name;
		$message = array($header->name => $header->type);
		$this->addMessage($messageName, $message);
		
		$soapHeader = $this->addElement("soap:header", $parent);
		$soapHeader->setAttribute("message", "tns:".$messageName);
		$soapHeader->setAttribute("part", $header->name);
		$soapHeader->setAttribute("use", ($this->use == SOAP_ENCODED)? "encoded" : "literal");
		$soapHeader->setAttribute("namespace", $this->tns);
		if($this->use == SOAP_ENCODED)
			$soapHeader->setAttribute("encodingStyle", self::NS_ENC);
	}
