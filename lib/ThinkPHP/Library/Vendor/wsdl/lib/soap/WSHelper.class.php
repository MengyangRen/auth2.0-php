...
class WSHelper {
	...
	public $base = null;
	public $subbase = null; // add by spark, for services in plugin
	 ...
	 * @return void
	 */
	public function handle($wsdl = null){
		if($wsdl || substr($_SERVER['QUERY_STRING'], -4) == 'wsdl'){
			$this->showWSDL();
	...
	private function createWSDL(){
		...
		$wsdl = new WSDLStruct($this->uri, $this->base . ($this->subbase ? $this->subbase : '') ."?class=".$this->name, $this->type, $this->use);
		$wsdl->setService($this->class);
		$wsdl->setHeaders($this->headers);
	...
	public function createDocumentation($template="templates/docclass.xsl")	{
		$template = dirname(dirname(dirname(__FILE__))) . '/' . $template;
		//print file_exists($template);
		if(!is_file($template))
