 ...
 */
function loadService($name = null, $create = false) {
    if (!class_exists('AppService')) {
        if (file_exists(APP_SERVICE_PATH . 'AppService.class.php')) {
            require(APP_SERVICE_PATH . 'AppService.class.php');
        }
    }

    $path = APP_PATH.MODULE_NAME.'/Service/';
    $hasService = substr($name, -7) == 'Service';
    $className = $hasService? $name : $name . 'Service';
    $class      =    MODULE_NAME.'\\'.'Service'.'\\'.$className;
    
    if (!is_null($name) && !class_exists($className)) {
        $file = $name . ($hasService ? '.class.php' : 'Service.class.php');

        if (file_exists($path . $file)) {
            require($path . $file);

            return $create ? new $class : true;
        }
        return false;
    } else {
        return $create ? new $class : true;
